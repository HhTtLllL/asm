///////////////////////////////////////////////////////////////
// File Name: shell/buildin_cmd.c
// Author: tt
// Email: tttt@xiyoulinux.org
// Created Time : 2020-11-04 18:27:50
// Description:
///////////////////////////////////////////////////////////////
...
/*将路径old_abs_path 中的 . 和 .. 转换为实际路径后存入 new_abs_path*/
    ...
    while(name[0]) {
        ...
        if(!strcmp("..", name)) {
            ...
            if(strcmp(new_abs_path, "/")) {
                ...
                //如果new_abs_path 不是"/",就拼接一个"/", 此处的判断是为了避免路径开头变成这样 //
                strcat(new_abs_path, "/");
            }
            strcat(new_abs_path, name);
        }//若 name_abs_path为当前目录".", 无需处理new_abs_path
...
/*将path处理成 不含..和.的绝对路径, 存储在final_path*/
...
void buildin_ls(uint32_t argc, char** argv) {
    ...
    if (NULL == pathname) {	 // 若只输入了ls 或 ls -l,没有输入操作路径,默认以当前路径的绝对路径为参数.
    ...
    if (FT_DIRECTORY == file_stat.st_filetype) {
