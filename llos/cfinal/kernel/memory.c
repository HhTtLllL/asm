...
/* 位图地址 
 * 因为0xc009f000  是内核主线程栈顶,0xc009e000 是内核主线程pcb 
 * 一个页框大小的位图可表示 128M内存,位图位置安排在地址 0xc009a000,这样本系统最大支持4个页框的位图,即512MB
 */
...
struct arena {
    ...
    struct mem_block_desc* desc;     //从arena关联的mem_block_desc 
    uint32_t cnt;
    bool large;                     //large为ture时，cnt表示的是页框数，否则为空闲mem_block 数量
};
...
//在m_pool 指向的物理内存池中分配1个物理页,成功则返回页框的物理地址,失败则返回NULL 
static void* palloc(struct pool* m_pool) {
    ...
    bitmap_set(&m_pool->pool_bitmap, bit_idx, 1);               //将此位bit_idx 置1 
...
void* malloc_page(enum pool_flags pf, uint32_t pg_cnt) {
     ...
     * 1.vaddr_get 在虚拟内存池中申请虚拟地址
     * 2.palloc在物理内存池中申请物理页
     * 3.通过page_tale_add 将以上得到的虚拟地址和物理地址在页表中完成映射
     */
    ...
    while(cnt-- > 0) {
        ...
        if (page_phyaddr == NULL) {                 //失败时要将曾经已经申请的虚拟地址和物理页全部回滚,在将来完成内存时再补充
...
//将地址vaddr 与 pf 池中的物理地址关联，仅支持一页空间分配 
...
/*安装 1页大小的vaddr, 专门针对fork时虚拟地址位图无需操作的情况
 *
 *缺少虚拟地址池位图的操作
 * */
...
void* sys_malloc(uint32_t size) {
    ...
    if(cur_thread->pgdir == NULL) {             //若为内核线程
        ...
        descs = k_block_descs;
    }else {                                     //用户进程pcb 中的pgdir会在为其分配页表时创建
    ...
    if(size > 1024) {
        ...
        if(a != NULL) {
            ...
            //对于分配的大块页框，将desc置为NULL,cnt置为页框数,large 置为true 
            a->desc = NULL;
            a->cnt = page_cnt;                              //这个arena 结构占用的页框数
            a->large = true;
            lock_release(&mem_pool->lock);
            
            return (void*)(a + 1);                          //跨过arena 大小(跨过一个 struct arena)，把剩下的内存返回,
        }else {
        ...
        //若申请的内存小于等于1024,可在各种规格的mem_block_desc 中去适配
        uint8_t desc_idx;
        ...
        //若mem_block_desc的free_list总已经没有可用的mem_block,就创新的arena提供mem_block
        if(list_empty(&descs[desc_idx].free_list)) {
            ...
            //对于分配的小内存，将desc置为相应内存块描述符,cnt 置为此arena可用的内存块数，large 置为false 
            ...
            //开始将arena 拆分成内存块，并添加到内存块描述符的free_list中
...
/*pfree ---------将物理地址pg_phy_addr 回收到物理内存池 -------------*/
void pfree(uint32_t pg_phy_addr) {

    struct pool* mem_pool;                              //指向物理内存池
    uint32_t bit_idx = 0;                               //物理地址在物理内存池中的偏移量
...
//去掉页表中虚拟地址vaddr 的映射，只去掉vaddr 对应的pte 
...
void mfree_page(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt) {
    ...
    /*确保待释放的物理内存在低端1MB + 1KB 大小的页目录 + 1KB大小的页表地址范围外 */
    ASSERT((pg_phy_addr % PG_SIZE) == 0 && pg_phy_addr >= 0x102000);

    /* 判断pg_phy_addr 属于用户物理内存池还是内核物理内存池 */
    if(pg_phy_addr >= user_pool.phy_addr_start) {               //用户内存池user_pool 
...
void sys_free(void* ptr) {
    ...
    if(ptr != NULL) {
        ...
        if(a->desc == NULL && a->large == true) {   //大于1024的内存
            ...
            /*先将内存块回收到free_list*/
            list_append(&a->desc->free_list, &b->free_elem);
            /*再判断此arena 中的内存块是否都是空闲，如果是就释放arena*/
...
void block_desc_init(struct mem_block_desc* desc_array) {
    ...
    //初始化每个mem_block_desc 描述符
    for(desc_idx = 0; desc_idx < DESC_CNT; desc_idx ++) {

        desc_array[desc_idx].block_size = block_size;
        //初始化arena中的内存数量
        desc_array[desc_idx].blocks_per_arena = (PG_SIZE - sizeof(struct arena)) / block_size;  //这个arena 中有多少个 这个规格的内存块
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    uint16_t all_free_pages = free_mem / PG_SIZE;       //对于以页为单位的内存分配策略,不足一页的内存不用考虑
    uint16_t kernel_free_pages = all_free_pages / 2;
    uint16_t user_free_pages = all_free_pages - kernel_free_pages;

    //为简化位图操作,余数不做处理,坏处是这样做会丢内存, 好处是不用做内存的越界检查,因为位图表示的内存少于实际物理内存
    ...
    kernel_pool.phy_addr_start = kp_start;                  //初始化各自内存池的起始地址
...
//内核内存池和用户内存池位图
//位图是全局的数据,长度不固定, 全局或静态的数组需要在编译时知道其长度, 而我们需要根据总内存大小算出需要多少字节,所以
//改为指定一块内存来生成位图   

//内核使用的最高地址是0xc009f000,这是主线程的栈地址, 内核的大小预计为 70kb 左右,32MB内存占用的位图是2kb
