///////////////////////////////////////////////////////////////
// File Name: userprog/fork.c
// Author: tt
// Email: tttt@xiyoulinux.org
// Created Time : 2020-11-03 20:23:01
// Description:
///////////////////////////////////////////////////////////////
...
static int32_t copy_pcb_vaddrbitmap_stack0(struct task_struct* child_thread, struct task_struct* parent_thread) {
    ...
    /*a 复制pcb所在的整个页, 里面包含进程pcb信息及特级0级的栈*/
    ...
    /*此时child_thread->userprog_vaddr.vaddr_bitmap.bits 还是指向父进程虚拟地址的位图地址
...
static int32_t build_child_stack(struct task_struct* child_thread) {
    ...
    /*b 为switch_to 构建struct thread_stack, 将其构建在紧临intr_stack之下的空间*/
    ...
    /*ebp 在thread_stack中的地址便是当时esp(0级栈的栈顶), 
     *即esp为 (uint32_t*)intr_0_stack - 5;
     * */
    uint32_t* ebp_ptr_in_thread_stack = (uint32_t*)intr_0_stack - 5;

    /*switch_to的返回地址更新为 intr_exit, 直接从中断返回*/
    *ret_addr_in_thread_stack = (uint32_t)intr_exit;

    /*下面这两行赋值只是为了使构建的thread_stack更加清晰,  不是必须, 因为在进入intr_exit后一系列的pop会把寄存器中的数据覆盖掉*/
    *ebp_ptr_in_thread_stack = *ebx_ptr_in_thread_stack = *edi_ptr_in_thread_stack = *esi_ptr_in_thread_stack = 0;

    /*把构建的thread_stack的栈顶作为switch_to 恢复数据时的栈顶*/
...
static int32_t copy_process(struct task_struct* child_thread, struct task_struct* parent_thread){
    ...
    if(-1 == copy_pcb_vaddrbitmap_stack0(child_thread, parent_thread)) return -1;

    /*b 为子进程创建页表, 此页表仅包括内核空间*/
    child_thread->pgdir = create_page_dir();
    if(NULL == child_thread->pgdir)  return -1;

    /*c 复制父进程体及用户栈给子进程*/
    copy_body_stack3(child_thread, parent_thread, buf_page);

    /*d 构建进程thread_stack 和修改返回值 pid*/
...
pid_t sys_fork(void) {
    ...
    if(-1 == copy_process(child_thread, parent_thread)) return -1;
