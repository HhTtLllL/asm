...
/*将目录项p_de写入父目录 parent_dir中,io_buf 由主调函数提供*/
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf) {

    struct inode* dir_inode = parent_dir->inode;                                    //取出目录的inode 
    uint32_t dir_size = dir_inode->i_size;                                          //目录的i_size是目录中目录项大小之和
    uint32_t dir_entry_size = cur_part->sb->dir_entry_size;                         //目录项的大小

    ASSERT(dir_size % dir_entry_size == 0);                                         //dir_size应该是dir_entry_size的整数倍

    uint32_t dir_entrys_per_sec = (512 / dir_entry_size);                            //一个扇区最多有多少个目录项
    int32_t block_lba = -1;
    /*将该目录的所有扇区地址(12个直接块 + 128个间接块)存入 all_blocks*/

    uint8_t block_idx = 0;
    uint32_t all_blocks[140] = {0};                                                 //all_blocks保存目录所有块
    ...
    /*开始遍历所有块以寻找目录项位,若已有扇区中没有空闲位,
     * 在不超过文件大小的情况下申请新扇区来存储新目录项*/
    block_idx = 0;
    while(block_idx < 140) {                                                        //文件(包括目录)最大支持12个直接块 + 128个间接块 = 140块

        block_bitmap_idx = -1;
        if(all_blocks[block_idx] == 0) {                                            //在三种情况下分配块

            block_lba = block_bitmap_alloc(cur_part);                               //分配一个扇区
            if(block_lba == -1) {
            ...
            /*因为操作位图操作的是硬盘中的位图,所以需要将其同步到硬盘上*/
            block_bitmap_idx = block_lba - cur_part->sb->data_start_lba;                        //计算出偏移
            ASSERT(block_bitmap_idx != -1);
            bitmap_sync(cur_part, block_bitmap_idx, BLOCK_BITMAP);                              //同步到硬盘上

            block_bitmap_idx = -1;
            if(block_idx < 12) {                                                                //若是直接块
                
                dir_inode->i_sectors[block_idx] = all_blocks[block_idx] = block_lba;            //将分配的扇区写入inode中
                ...
                all_blocks[12] = block_lba;                                                     //把新分配的第0个间接块地址写入一级间接块表中
        ...
        while(dir_entry_idx < dir_entrys_per_sec) {                                 //遍历这个扇区中的所有目录项
    
            if((dir_e + dir_entry_idx)->f_type == FT_UNKNOWN) {

                //FT_UNKNOWN为0,无论是初始化或是删除文件后,都会将f_type置为 FT_UNKNOWN 
...
/*把分区 part目录pdir中编号为inode_no的目录项删除*/
bool delete_dir_entry(struct partition* part, struct dir* pdir, uint32_t inode_no, void* io_buf)  {
    ...
    while(block_idx < 140) {
        ...
        if(dir_entry_cnt == 1 && !is_dir_first_block) {
            ...
            if(block_idx < 12) {
                ...
                /*先判断一级间接索引表中间接块的数量,如果仅有1个间接块,连同间接索引表所在的块一同回收*/
                ...
                if(indirect_blocks > 1) {
                    ...
                    /*间接索引表中还包括其他间接快,仅在索引表中擦除当前这个间接快地址*/
    ...
    /*所有块中未找到则返回false, 出现这种情况应该是 search_file 出错*/
    return false;
}

/*读取目录,成功返回1个目录项,失败返回 NULL*/ 
struct dir_entry* dir_read(struct dir* dir) {
    ...
    if(dir_inode->i_sectors[12] != 0) {                             //若含有一级间接快
...
int32_t dir_remove(struct dir* parent_dir, struct dir* child_dir) {
    ...
    /*在父目录parent_dir 中删除子目录child_dir对应的目录项*/
    delete_dir_entry(cur_part, parent_dir, child_dir_inode->i_no, io_buf);

    /*回收inode 中 i_sectors 中所占用的扇区并同步 inode_bitmap 和 block_bitmap*/
